...
std::shared_ptr<Renderer::Texture2D> ResourceManager::loadTexture(const std::string& textureName, const std::string& texturePath)
{
	...
	std::shared_ptr<Renderer::Texture2D> newTexture = m_textures.emplace(
		textureName, std::make_shared<Renderer::Texture2D>(
	...
	const unsigned int subTextureHeight)
{
	...
	if (pTexture)
	{
		...
		for (const auto& currentSubTextureName : subTextures)
		{
			...
			glm::vec2 rightTopUV(static_cast<float>(currentTextureOffsetY + subTextureWidth) / textureWidth, static_cast<float>(currentTextureOffsetY) / textureHeight);
